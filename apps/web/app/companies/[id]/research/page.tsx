...
export default async function ResearchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Search> }) {
  ...
  const back = `/companies/${cid}${as_of ? `?as_of=${as_of}` : ""}`;
  const ratingHistory = signals.data.filter((s) => s.signal_type.startsWith("credit_rating"));

  return (
    <div className="space-y-6">
      <div className="text-[13px]"><Link href={back}>← Back to the summary</Link></div>
      {/* 1. Header */}
